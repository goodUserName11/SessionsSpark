...
class SessionEventParserSpec extends AnyFlatSpec with Matchers {
  "SessionEventParser" should "correctly parse a valid input string" in {
    ...
    val expectedObject = SessionEvent(timestamp, timestamp, events, problems, "SESSION")

    val resultDoc = sessionParser.parse(inputString, datetimePattern)
  ...
  it should "handle missing field gracefully" in {
    ...
    val events = List[SearchEvent]()
    val expectedObject = SessionEvent(timestamp, endTimestamp, events, problems, "SESSION")
